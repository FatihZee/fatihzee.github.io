document.addEventListener("DOMContentLoaded", function () {
  ...
  function showThemeNotification(message, icon) {
    const isDark =
      document.documentElement.getAttribute("data-theme") === "dark";

    themeNotification.innerHTML = `<i class="${icon}"></i> ${message}`;
    themeNotification.classList.add("show");

    if (isDark) {
      themeNotification.style.backgroundColor = "rgba(0, 0, 0, 0.9)";
      themeNotification.style.color = "white";
    } else {
      themeNotification.style.backgroundColor = "rgba(44, 62, 80, 0.9)";
      themeNotification.style.color = "white";
    }
  ...
  darkModeToggle.addEventListener("click", function () {
    ...
    if (isDark) {
      removeDarkMode();
      showThemeNotification("Switched to Light Mode", "fas fa-sun");
    } else {
      applyDarkMode();
      showThemeNotification("Switched to Dark Mode", "fas fa-moon");
    }
  });
  ...
  function applyDarkMode() {
    ...
    document
      .querySelectorAll(".experience-duration, .education-duration")
      .forEach((el) => {
        el.style.transition = "background-color 0.5s ease, color 0.5s ease";
  ...
  function removeDarkMode() {
    ...
    document
      .querySelectorAll(".experience-company, .education-institution")
      .forEach((el) => (el.style.color = "#3498db"));

    document
      .querySelectorAll(".project-tech")
      .forEach((el) => (el.style.color = "#3498db"));
